...
test("staticAnalysisCommon.helpers.buildFunctionSignature", function (t) {
  ...
  t.equal(
    common.helpers.buildFunctionSignature([common.basicTypes.UINT, common.basicTypes.ADDRESS], [common.basicTypes.BOOL], false),
    "function (uint256,address) returns (bool)",
    "two params and return value without payable"
  )

  t.equal(
    common.helpers.buildFunctionSignature([common.basicTypes.UINT, common.basicTypes.ADDRESS], [common.basicTypes.BOOL], false, "pure"),
    "function (uint256,address) pure returns (bool)",
    "two params and return value without payable but pure"
  )

  t.equal(
    common.helpers.buildFunctionSignature([common.basicTypes.UINT, common.basicTypes.ADDRESS], [common.basicTypes.BOOL], true, "pure"),
    "function (uint256,address) payable pure returns (bool)",
    "two params and return value without payable but pure"
  )

  t.equal(
    common.helpers.buildFunctionSignature([common.basicTypes.UINT, common.basicTypes.BYTES32, common.basicTypes.BYTES32], [], true),
    "function (uint256,bytes32,bytes32) payable",
    "three params and no return with payable"
  )

  t.equal(
    common.helpers.buildFunctionSignature([common.basicTypes.BOOL], [common.basicTypes.BYTES32, common.basicTypes.ADDRESS], true),
    "function (bool) payable returns (bytes32,address)",
    "one param and two return values with payable"
  )

  t.equal(common.lowLevelCallTypes.CALL.type, "function (bytes memory) payable returns (bool,bytes memory)", "check fixed call type")

  t.equal(common.lowLevelCallTypes["CALL-0.4"].type, "function () payable returns (bool)", "check fixed call type for versions before 0.5.0")

  t.equal(common.lowLevelCallTypes.CALLCODE.type, "function () payable returns (bool)", "check fixed callcode type")

  t.equal(common.lowLevelCallTypes.SEND.type, "function (uint256) returns (bool)", "check fixed send type")

  t.equal(common.lowLevelCallTypes.DELEGATECALL.type, "function (bytes memory) returns (bool,bytes memory)", "check fixed delegatecall type")

  t.equal(common.lowLevelCallTypes["DELEGATECALL-0.4"].type, "function () returns (bool)", "check fixed delegatecall type for version before 0.5.0")
})
...
test("staticAnalysisCommon.getEffectedVariableName", function (t) {
  ...
  t.throws(
    () => common.getEffectedVariableName(inlineAssembly),
    new RegExp("staticAnalysisCommon.js: wrong node type"),
    "staticAnalysisCommon.js: not an effect Node or inline assembly, get from inline assembly should throw"
  )
...
test("staticAnalysisCommon.getFullQuallyfiedFuncDefinitionIdent", function (t) {
  ...
  t.throws(
    () => common.getFullQuallyfiedFuncDefinitionIdent(contractDefinition, parameterFunctionCall, ["uint256", "bool"]),
    new RegExp("staticAnalysisCommon.js: not a FunctionDefinition Node"),
    "throws on wrong nodes"
  )
  t.throws(
    () => common.getFullQuallyfiedFuncDefinitionIdent(parameterFunctionCall, functionDefinition, ["uint256", "bool"]),
    new RegExp("staticAnalysisCommon.js: not a ContractDefinition Node"),
    "throws on wrong nodes"
  )
})
...
test("staticAnalysisCommon.getSplittedTypeDesc", function (t) {
  ...
  t.ok(
    common.getMethodParamsSplittedTypeDesc(funcDefForComplexParams.nestedStruct, compiledContractObj)[0] === "(bytes32,uint256,uint256[],address,(bytes32,uint256)[])[][]",
    "creates right params type signature"
  )
})
